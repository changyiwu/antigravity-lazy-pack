...
## 新專案初始化
先問：名稱、用途、資料夾、是否 GitHub repo、公開/私有、是否部署、Obsidian vault 位置。
建立：ANTIGRAVITY.md、README.md、.gitignore、Git repo、GitHub repo、Obsidian 專案駕駛艙（專案筆記明確寫到 Obsidian 根目錄底下，檔名格式為 `<專案名稱>-專案駕駛艙.md`）。
若已存在 → 盤點後只補缺口，不覆蓋。
